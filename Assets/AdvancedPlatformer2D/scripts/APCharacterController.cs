...
public partial class APCharacterController : MonoBehaviour
{
	...
	bool m_attackNoMove;
	APAttack.ContextId m_rangedContextId;
	...
	public VirtualButton ShiftButton;
	...
	void Start()
	{
		...
		if (JumpButton != null)
		{
			m_jump.m_button.m_virtualButton = JumpButton;
			JumpButton.OnButtonPressed += () => m_jump.m_button.Refresh(true);
			JumpButton.OnButtonReleased += () => m_jump.m_button.Refresh(false);
		}

		if (AttackButton != null)
		{
			foreach (var attack in m_attacks.m_attacks)
			{
				attack.m_button.m_virtualButton = AttackButton;
				AttackButton.OnButtonPressed += () => attack.m_button.Refresh(true);
				AttackButton.OnButtonReleased += () => attack.m_button.Refresh(false);
			}
		}

		if (ShiftButton != null)
		{
			foreach (var shift in m_shift.m_shifts)
			{
				shift.m_button.m_virtualButton = ShiftButton;
				ShiftButton.OnButtonPressed += () => shift.m_button.Refresh(true);
				ShiftButton.OnButtonReleased += () => shift.m_button.Refresh(false);
			}
		}
	}
	...
	void ClearRuntimeValues()
	{
		...
		m_sliding = false;
		m_curAttack = null;
		m_rangedContextId = APAttack.ContextId.eStand;
	...
	void RefreshInputs(bool bSet)
	{
        ...
        {
            if ((Time.time >= m_timePushedJumpAxis + APInputButton.m_downPushDuration))
            {
                m_hasPushedJumpAxis = false;
            }
        }
    ...
    bool InitAttack(APAttack newAttack)
    {
        ...
        APAttack.AttackContext curCtx = null;
        bool bCrouched = IsCrouched();
        if (bCrouched)
        {
            curCtx = newAttack.m_contextCrouched;
            m_rangedContextId = APAttack.ContextId.eCrouched;
        }
        else if (m_onGround)
        {
            if (m_isInStand)
            {
                curCtx = newAttack.m_contextStand;
                m_rangedContextId = APAttack.ContextId.eStand;
            }
            else
            {
                // currently running
                curCtx = newAttack.m_contextRun;
                m_rangedContextId = APAttack.ContextId.eRun;
            }
        }
        else
        {
            curCtx = newAttack.m_contextInAir;
            m_rangedContextId = APAttack.ContextId.eInAir;
        }
        ...
        m_attackNoMove = curCtx.m_stopOnGround;
        ...
        if (!string.IsNullOrEmpty(curCtx.m_anim))
        {
            m_curAttackAnimHash = Animator.StringToHash(curCtx.m_anim);
        }
        else
        {
            m_curAttackAnimHash = 0;
            ...
            case APFsmStateEvent.eUpdate:
                {
                    ...
                    if ((fAnimTime >= 1f) && !IsNewStateRequested())
                    {
                        ...
                        if (m_curAttack.m_button.GetButtonDown() && InitAttack(m_curAttack))
                        {
                            SetState(State.Attack);
                        }
                        else
	...
	void FireRangedAttack()
	{
		...
		{
            m_curAttack.FireBullet(this, m_rangedContextId);
		}
	}
