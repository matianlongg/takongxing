...
public class APCharacterControllerEditor : Editor
{
            ...
            foreach (APAttack curAttack in oController.m_attacks.m_attacks)
			{
				foreach (APHitZone curZone in curAttack.m_hitZones)
				{
					if (curZone.m_active && curZone.gameObject.activeInHierarchy)
					{
						...
						color.a = 0.5f;
						Handles.color = color;
						var fmh_67_57_638633061706359190 = Quaternion.identity; Vector3 newPos = Handles.FreeMoveHandle(pointPos, curZone.m_radius * 2f, Vector3.zero, Handles.SphereHandleCap);
						if (newPos != pointPos)
						{
	...
	public void DrawStartPos(APAttack.AttackContext context, bool bDraw)
	{
		if(bDraw && !string.IsNullOrEmpty(context.m_anim))
		{
			...
			color.a = 0.5f;
			Handles.color = color;
			var fmh_104_54_638633061706399428 = Quaternion.identity; Vector3 newPos = Handles.FreeMoveHandle(pointPos, 0.1f, Vector3.zero, Handles.SphereHandleCap);
			if (GUI.changed)
			{
